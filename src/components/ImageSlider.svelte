...
<style lang="scss">
  :global(.splide__slide) {
    figure {
      ...
      figcaption {
        @apply absolute top-0 left-0 w-full h-full flex items-end bg-slate-950/80 p-4;
        ...
        h3 {
          transition: opacity 2s ease-in-out;
          @apply font-bold text-lg text-white relative z-10 opacity-0;
          @screen md {
            @apply text-3xl;
          }
        }
  ...
  :global(.splide__arrow) {
    @apply w-auto h-auto p-2 bg-white/30 backdrop-blur-md text-xl pointer-events-auto text-primary;
    @screen md {
      @apply p-8 text-3xl text-white;
    }
  }
</style>
